...
*/
	...
	KIPS = (100.0*LOOP*II)/(float)(finisec-startsec)*1000;
	if (KIPS >= 1000.0)
		printf("C Converted " PRECSTR " Precision Whetstones: %.4f MIPS\n", KIPS/1000.0);
	else
		printf("C Converted " PRECSTR " Precision Whetstones: %.4f KIPS\n", KIPS);
...
POUT(long N, long J, long K, FLOAT X1, FLOAT X2, FLOAT X3, FLOAT X4)
{
  long t1, t2;
  t1 = clock ();
  printf("%7ld %7ld %7ld %12.4e %12.4e %12.4e %12.4e %ld\n",
	 N, J, K, X1, X2, X3, X4, t1 - adjust - startsec);
  t2 = clock ();
  adjust += (t2 - t1);
}
#endif
